...
def get_docker_ami(**kwargs):
    ...
    if 'ec2_client' not in kwargs:
        raise Exception('Missing required parameter ec2_client')
...
def get_subnet_ids(**kwargs):
    """get the subnet id of  with 'Environment_Name:environment_name_subnet_tag' and
    ...
    Keyword arguments:
    ec2_client (required) -- an amazon BOTO3 EC2 client
    environment_name_subnet_tag (required) -- environment name
    network_segment_subnet_tag -- default 'MW'
    ...
    if 'environment_name_subnet_tag' not in kwargs:
        raise Exception('Missing required parameter environment_name_subnet_tag')

    ec2_client = kwargs.get('ec2_client')
    environment_name = kwargs.get('environment_name_subnet_tag')
    network_segment = kwargs.get('network_segment_subnet_tag', 'MW')
...
def get_next_available_name(**kwargs):
    ...
    Keyword arguments:
    ec2_client (required) -- an amazon BOTO3 EC2 client
    base_instance_name (required) -- environment name
    """

    if 'ec2_client' not in kwargs:
        raise Exception('Missing required parameter ec2_client')

    if 'base_instance_name' not in kwargs:
        raise Exception('Missing required parameter base_instance_name')

    ec2_client = kwargs.get('ec2_client')
    base_name = kwargs.get('base_instance_name')
    ...
    count = 1
    while base_name+'0a'+format(count, '02d') in names:
        count += 1
    new_name = base_name+'0a'+format(count, '02d')
...
def create_instance(**kwargs):
    ...
    if 'ami_id' not in kwargs:
        raise Exception('Missing required parameter ami_id')

    if 'env_name' not in kwargs:
        raise Exception('Missing required parameter env_name')
    ...
    instance_name = get_next_available_name(
        ...
        base_instance_name=env_name+'-'+network_segment+'-docker'
    )
